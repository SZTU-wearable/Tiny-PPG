...
class ContrastLoss(nn.Module, ABC):
    ...
    def _contrastive(self, feats_, labels_):
        ...
        anchor_dot_contrast = torch.div(torch.matmul(anchor_feature, torch.transpose(contrast_feature, 0, 1)), #Dot product of all feature vectors
                                        self.temperature)  
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True) 
        logits = anchor_dot_contrast - logits_max.detach()  #smooth
